import { Bell, Search, Command, User, Menu } from 'lucide-react';
    ...
    <header className="h-16 border-b border-white/5 flex items-center justify-between px-4 lg:px-8 bg-black/50 backdrop-blur-md sticky top-0 z-30 w-full">
      <div className="flex items-center gap-4 flex-1">
        ...
        <div className="relative max-w-md w-full group hidden sm:block">
          ...
          <input 
            type="text" 
            placeholder="Search OS: Commands, Customers, Orders..." 
            className="w-full bg-white/5 border border-white/5 rounded-xl pl-10 pr-4 py-2 text-sm text-white placeholder:text-neutral-600 focus:outline-none focus:ring-1 focus:ring-blue-500/50 transition-all"
          />
      ...
      <div className="flex items-center gap-3 lg:gap-6">
        <div className="hidden md:flex items-center gap-4 pr-6 border-r border-white/10">
          <div className="flex flex-col items-end">
            ...
            <span className="text-[9px] text-neutral-600 font-mono">12.4</span>
          </div>
        ...
        <div className="flex items-center gap-2 lg:gap-3">
          <div className="hidden sm:flex flex-col items-end">
            <p className="text-sm font-medium text-white truncate max-w-[100px] lg:max-w-none">{user?.displayName || 'User'}</p>
            <p className="text-[10px] text-neutral-500 truncate max-w-[100px] lg:max-w-none">{user?.email}</p>
          </div>
          <div className="w-9 h-9 lg:w-10 lg:h-10 rounded-full bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center border border-white/20 shadow-lg group cursor-pointer hover:scale-105 transition-transform shrink-0">
